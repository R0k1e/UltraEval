...
def omg_eval(models, languages, mode='all'):
    ...
    for model in os.listdir(instance_path):
        ...
        for config_tag in os.listdir(os.path.join(instance_path, model)):
            cur_path = os.path.join(instance_path, model, config_tag)
            for model_tag in os.listdir(cur_path):
                for exec_date in os.listdir(os.path.join(instance_path, model, config_tag, model_tag)):
                    cur_path = os.path.join(instance_path, model, config_tag, model_tag, exec_date)
                    for task_name in os.listdir(cur_path):
                        cur_path = os.path.join(instance_path, model, config_tag, model_tag, exec_date, task_name)
...
def omg_eval_old(models, languages, mode='all'):
    instance_path = "./old-result/omgeval"
    for model in os.listdir(instance_path):
        if model not in models:
            continue
        cur_path = os.path.join(instance_path, model)
        for exec_date in os.listdir(os.path.join(instance_path, model)):
...
if __name__ == '__main__':
    ...
    if version == "new":
        ...
        omg_eval_old(models, languages, mode)