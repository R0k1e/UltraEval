...
def omg_eval(models, languages, mode='all'):
    ...
    for model in os.listdir(instance_path):
        ...
        for model_tag in os.listdir(cur_path):
            for exec_date in os.listdir(instance_path, model, model_tag):
                cur_path = os.path.join(instance_path, model, model_tag, exec_date)
                for task_name in os.listdir(cur_path):
                    cur_path = os.path.join(instance_path, model, model_tag, exec_date, task_name)
